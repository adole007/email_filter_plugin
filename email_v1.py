...
class EmailFilter:
  def __init__(self):
    # Load the dataset of emails containing the keywords
  ...
  def classify(self, emails):
    # Use the model to classify the input emails as either containing the keywords or not
...
# Create an instance of the EmailFilter plugin
filter = EmailFilter()

# Classify a list of new incoming emails
results = filter.classify(new_emails)

# Move the emails containing the keywords to their respective folders
for email, prediction in results:
  if prediction >= 0.5:
    if "loan" in email:
      move_to_folder(email, "Loan Emails")
    elif "rent" in email:
###not yet completed
